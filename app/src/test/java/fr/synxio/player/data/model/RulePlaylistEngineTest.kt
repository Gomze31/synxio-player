...
class RulePlaylistEngineTest {
    ...
    private fun rules(
        ...
        limit: Int? = null,
    ) = RulePlaylistRules(rule.toList(), matchAll, sort, descending, limit)
    ...
    fun `contains est insensible a la casse`() {
        ...
        val result = RulePlaylistEngine.evaluate(
    ...
    // lastPlayedAt est en MILLISECONDES (PlayStatEntity)
    @Test
    ...
    fun `not in last days ignore les morceaux jamais joues`() {
        ...
        // une règle « pas écouté depuis 180 jours » ne doit pas se remplir de morceaux
        // jamais écoutés.
        val songs = listOf(song(id = 1), song(id = 2))
    ...
    fun `aucune regle retourne toute la bibliotheque`() {
        val songs = listOf(song(id = 1), song(id = 2))
        val result = RulePlaylistEngine.evaluate(rules(), songs, emptyMap(), emptySet(), now)
    ...
    fun `le tri aleatoire est stable pour un meme jour`() {
        ...
        val a = RulePlaylistEngine.evaluate(r, songs, emptyMap(), emptySet(), now)
        val b = RulePlaylistEngine.evaluate(r, songs, emptyMap(), emptySet(), now + 1000)
    ...
    fun `les regles survivent a un aller-retour json`() {
        ...
        assertEquals(original, RulePlaylistRules.fromJson(original.toJson()))
    }

    @Test
    fun `un json corrompu retombe sur des regles vides`() {
        assertEquals(RulePlaylistRules(), RulePlaylistRules.fromJson("{pas du json"))
    }
}
